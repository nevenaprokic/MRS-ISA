...
import { getAllCottages, deleteCottageByAdmin } from "../../services/CottageService";
import { getAllShips, deleteShipByAdmin } from "../../services/ShipService";
import { getAllAdventures, deleteAdventureByAdmin } from "../../services/AdventureService";
      ...
      <ThemeProvider theme={theme}>
        <React.Fragment>
          <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
            ...
            <TableCell sx={{ fontSize: 16 }} align="center" >{offer.name ? offer.name : offer.offerName}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{offer.price}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{offer.peopleNum ? offer.peopleNum : offer.numberOfPerson}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{offer.ownerName}</TableCell>
            <TableCell ><Button
              variant="contained"
              sx={{ float: "right" }}
              color="primary"
...
function OffersOverview() {
  ...
  function setTableData(offer_type, selectedPage, selectedPageSize) {
    async function set() {
      ...
      const response = await getOffersFunction[offer_type](selectedPage, selectedPageSize);
      setOffers(!!response.data ? response.data : {});
      setOfferNumber(!!response.data.length > 0 ? response.data[0].offerNumber : 0);
      return response;
    }
    set();
  }
    ...
    return (
      <div className="requestsC ontainer">
        <Typography variant="h6" sx={{ color: "#5f6d5f" }}>
          As an administrator, you can delete any offer if it doesn't have future reservations.
        </Typography>
        <Divider />
        <br />
        ...
        <Autocomplete
          ...
          options={offerTypes}
          sx={{ width: 250 }}
          onChange={(event, newValue) => { hadnleOfferTypeCange(event, newValue) }}
          ...
          <Table aria-label="collapsible table" >
            ...
            <TableBody>
              ...
              {offers.map((row) => (
                ...
                <Row key={row.id} offer={row} setOffers={setOffers} allOffers={offers} typeOfOffer={selectedOfferType} />
              ))}
            </TableBody>
          </Table>
        </TableContainer>
        ...
        />
        {offersNumber === 0 && <Typography variant="h6" sx={{ color: "#CC7351" }}>There are currently no offers for selected type</Typography>}
        </>}
        </div>
    );
  } else return null;
