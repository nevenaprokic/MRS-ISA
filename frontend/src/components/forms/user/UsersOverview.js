...
import { getAllInstructors, deleteInstructor } from "../../../services/InstructorService";
import { getAllCottageOwners, deleteCottageOwner } from "../../../services/CottageOwnerService";
import { getAllShipOwner, deleteShipOwner } from "../../../services/ShipOwnerService";
      ...
      <ThemeProvider theme={theme}>
        <React.Fragment>
          <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
            ...
            <TableCell sx={{ fontSize: 16 }} align="center" >{user.firstName}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{user.lastName}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{user.role}</TableCell>
            <TableCell sx={{ fontSize: 16 }} align="center">{user.category}</TableCell>
            <TableCell ><Button
              type="submit"
              variant="contained"
              sx={{ float: "right" }}
            ...
            <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
              <Collapse in={open} timeout="auto" unmountOnExit>
                <Box sx={{ margin: 1 }}>
                  ...
                  <Typography variant="body1" gutterBottom component="div" sx={{ color: "#5f6d5f" }}>
                    Address:
                    <label className="textItem"> {`${user.street}, ${user.city}, ${user.state}`} </label>
                  </Typography>
...
function UsersOveview() {
  ...
  function setTableData(role, selectedPage, selectedPageSize) {
    async function set() {
      console.log(role);
      const response = await getUsersFunction[role](selectedPage, selectedPageSize);
      setUsers(!!response.data ? response.data : {});
      setUserNumber(!!response.data.length > 0 ? response.data[0].userNumber : 0);
      return response;
    }
    set();
  }
    ...
    return (
      <div className="requestsC ontainer">
        <Typography variant="h6" sx={{ color: "#5f6d5f" }}>
          As an administrator, you can delete any account if it doesn't have future reservations.
        </Typography>
        <Divider />
        <br />
          ...
          <Table aria-label="collapsible table" >
            <TableHead>
              ...
              <TableRow sx={{ borderBottom: "solid #99A799" }}>
                <TableCell><img width="60" height={"60"} src={removeIcon}></img> </TableCell>
            ...
            <TableBody>
              ...
              {users.map((row) => (
                ...
                <Row key={row.id} user={row} setUsers={setUsers} allUsers={users} />
              ))}
            </TableBody>
          </Table>
        </TableContainer>
        ...
        />
        {userNumber === 0 && <Typography variant="h6" sx={{ color: "#CC7351" }}>There are currently no users for selected type{getRoleFromToken() === userType.ADMIN && ", except you"}</Typography>}
        </>}
        </div>
    );
  } else return null;
